...
inline long double multivariate_gaussian(double mu_x, double mu_y, double std_x,
                                    double std_y, double x, double y) {
  return (1 / (2 * M_PI * std_x * std_y)) * exp(
    -(pow((x - mu_x), 2) / (2 * pow(std_x, 2)) + pow((y - mu_y), 2) / (2 * pow(std_y, 2)))
  );
}
